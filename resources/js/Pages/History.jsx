...
export default function History({ auth }) {
    ...
    const fetchHistories = async () => {
        try {
            const response = await axios.get(route('history.index'));
    ...
    const deleteHistory = async (id) => {
        try {
            await axios.delete(route('history.destroy', id));
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-primary leading-tight">Your Playlist History</h2>}
        >
            <Head title="History" />
                                        ...
                                        <div className="flex-1 p-4 lg:p-6">
                                            <div className="flex items-start justify-between mb-4">
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => openSpotifyPlaylist(history.playlist_url)}
                                            ...
                                            <p className="text-sm text-gray-400">
                                                Saved {new Date(history.created_at).toLocaleDateString()} at {new Date(history.created_at).toLocaleTimeString()}
                                            </p>
                                        </div>
                                    </div>
                                </div>
